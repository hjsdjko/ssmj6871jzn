...
<div :style='{"width":"100%","padding":"30px 7% 40px","margin":"0px auto","position":"relative","background":"#edeff1"}'>
    <el-form
      ...
      label-width="150px"
    >
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="捐赠编号" prop="juanzengbianhao">
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}'  label="物品分类" prop="wupinfenlei">
            <el-select v-model="ruleForm.wupinfenlei" placeholder="请选择物品分类" :disabled=" false  ||ro.wupinfenlei" >
              <el-option
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="物品来源" prop="wupinlaiyuan">
            <el-input v-model="ruleForm.wupinlaiyuan" 
                placeholder="物品来源" clearable :disabled=" false  ||ro.wupinlaiyuan"></el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="捐赠时间" prop="juanzengshijian">
              <el-date-picker
          ...
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="捐赠原因" prop="juanzengyuanyin">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="捐赠原因"
              v-model="ruleForm.juanzengyuanyin">
            </el-input>
          </el-form-item>
          <el-form-item :style='{"width":"100%","padding":"10px","margin":"0 0 10px","background":"none","display":"inline-block"}' label="详细说明" prop="xiangxishuoming">
            <editor 
...
<script>
  export default {
      ...
      init(type) {
        ...
        if(type=='cross'){
          ...
          for (var o in obj){
            ...
            if(o=='wupinlaiyuan'){
              this.ruleForm.wupinlaiyuan = obj[o];
              this.ro.wupinlaiyuan = true;
              continue;
            }
            if(o=='juanzengyuanyin'){
              this.ruleForm.juanzengyuanyin = obj[o];
              this.ro.juanzengyuanyin = true;
              continue;
            }
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`aixinjuanzeng/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('aixinjuanzeng/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`aixinjuanzeng/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
